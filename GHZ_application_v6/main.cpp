...
elapsed = ((end.tv_sec*1000000.0 + end.tv_usec) -
    ...
    printf("number stars: %llu\n", num_stars_glbl);
    printf("number of SNII: %llu\n", num_sn_glbl);
    printf("number of SNIa: %llu\n", num_sn_Ia_glbl);
    printf("total stellar mass: %4.7Lf\n", total_stellar_mass_glbl);
    
    
    printf("Sterilization distance multiplier: %d\n", ster_dist_multi);
    printf("Number density type: %d\n", number_density_type);
     ...
     //eg 1->1, 1->2, 2->1
         ...
         //*************************************
                     if(star1[i].sn==1 || star1[i].sn==2)
                     {
                         ...
                         long int upper = get_star_range_upper(i);
                              //for (long int j=get_star_range_lower(i); j<=get_star_range_upper(i); ++j)
                              for (long int j=get_star_range_lower(i); j<=upper; ++j)
                              {
     ...
     //eg 1->1, 1->2, 2->1
         ...
         //*************************************
                     if(star1[i].sn==1 ||star1[i].sn==2)
                     {
                         ...
                         long int range_upper = get_star_range_upper(i);
                         //for (long int j=get_star_range_lower(i); j<=get_star_range_upper(i); ++j)
                              for (long int j=get_star_range_lower(i); j<=range_upper; ++j)
                              {
                                  normal_ster_flag=0;
                                  wrap_ster_flag=0;
                                  birthdate_candidate=star1[j].birth_date;
         ...
         //*************************************                      
                         ...
                         long int temp1 = get_star_range_upper_cell_right(star1[i].cell);
                         //for (long int k=get_star_range_lower_cell_right(star1[i].cell); k<=get_star_range_upper_cell_right(star1[i].cell); ++k)
                              for (long int k=get_star_range_lower_cell_right(star1[i].cell); k<=temp1; ++k)
                              {
                                  normal_ster_flag=0;
                                  wrap_ster_flag=0;
...
void create_columns()
{
    ...
    FILE *column_test;
    column_test = fopen("column_test.txt", "w");
...
column_array = (struct column_properties **)realloc(column_array, (count_xy_glbl+1) * sizeof(struct column_properties *));            
     ...
     for (int l=1; l<=count_xy_glbl; ++l)
    {
        ...
        fprintf(column_test, "%d, %d, %ld\n", column_array[l]->cell_min, column_array[l]->cell_max,column_array[l]->num_stars);
...
void create_subsections()
{
    printf("Creating Model Structure (Cells, Columns etc.\n)");
...
void output_stats()
{
    ...
    fprintf(GHZ_output, "column_number, radial distance, total stars, total sterilizations- at least sterilized once\n");
    ...
    fprintf(GHZ_output, "The following results are multiplied by 360 such that they apply to the entire Galaxy\n Number of TypeII: %llu", num_sn_glbl*360);
    fprintf(GHZ_output, " Number of TypeIa: %llu", num_sn_Ia_glbl*360);
    fprintf(GHZ_output, " Total Sterilizations SNII: %lu", total_sterilizations_II*360);
